...
- [Feature: Hotel Info gate (branch feature/ticket-info-hotel-room)](feature_hotel_info_gate.md) — TWO granular flags on `company.guest_page_cfg`, deployed + validated in dev 2026-08-04. Firebase RT DB sync from `guestRequestCar` implemented same day (commit `0e31687a` on ms-valet-service, merged to `feature/staging` as `a2a4cbd5`) — extends EDIT_TICKET_INFO snapshot with `hotelRoom`/`expectedDeparture`, pending user validation of end-to-end mobile visibility
...
- [Branching model — este repo usa develop](project_branching_model.md) — La regla completa está en shared/feedback_branching_model.md; aquí solo: la rama de pruebas es `develop` y hay que compilar después de mergear
...
- [Feature: link al recibo en Guest Page](feature_stripe_receipt_url.md) — Link a receipts.corepark.com/ticket/{parking_service.uuid}, sirve para los 4 gateways; el nodo `receipt` de ticket-info lo habilita; incluye la carrera contra el webhook
