...
**Excepción — iteración activa sobre una feature:** cuando estamos iterando en múltiples pasos sobre una misma feature (el usuario dice "vamos iterando", "de momento", "lo demás lo manejamos después", etc.), NO cerrar cada respuesta empujando hacia commit. El aviso "sin commitear" solo aplica al final del ciclo, cuando el usuario declara "primera versión funcionando" o autoriza commit. Empujar hacia commit después de cada micro-cambio rompe el flujo iterativo. El riesgo de merge accidental sigue existiendo, pero durante iteración activa esto se cubre con: (a) la rama es feature branch aislada, (b) el usuario está enterado del working tree porque él mismo dirige la iteración.
